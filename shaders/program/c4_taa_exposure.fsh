...
vec3 neighborhood_clipping(
	...
	// Using combined reinhard_to_ycocg function to reduce function call overhead
...
void main() {
	...
	// Clamp previous UV to valid range and detect off-screen pixels
	vec2 previous_uv_clamped = clamp(previous_uv, vec2(view_pixel_size), 1.0 - view_pixel_size);
	bool off_screen = any(notEqual(previous_uv, previous_uv_clamped));

	vec3 history_color = catmull_rom_filter_fast_rgb(colortex5, previous_uv_clamped, 0.6);
	     history_color = max0(history_color); // Eliminate NaNs in the history

	float pixel_age = texelFetch(colortex5, ivec2(previous_uv_clamped * view_res), 0).a;
	      pixel_age = off_screen ? 1.0 : max0(pixel_age + 1.0);
	...
	// Bypass TAA/TAAU history blending for hand to prevent trailing/ghosting during camera rotation
	if (hand) alpha = 1.0;

	// Offcenter rejection from Jessie, which is originally by Zombye
	// Reduces blur in motion
	vec2 pixel_offset = 1.0 - abs(2.0 * fract(view_res * previous_uv_clamped) - 1.0);
