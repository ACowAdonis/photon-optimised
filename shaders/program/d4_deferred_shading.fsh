...
void main() {
	...
	// Latitude-adjusted sun/moon/light directions
#ifdef LATITUDE_SUN_PATH
	vec3 adjusted_sun_dir = get_latitude_adjusted_sun_dir(sun_dir, cameraPosition.z, yearProgress);
	vec3 adjusted_moon_dir = -adjusted_sun_dir;
	vec3 adjusted_light_dir = sunAngle < 0.5 ? adjusted_sun_dir : adjusted_moon_dir;
#else
	#define adjusted_sun_dir sun_dir
	#define adjusted_moon_dir moon_dir
	...
	if (depth == 1.0) { // Sky
...
#if defined WORLD_OVERWORLD && defined BORDER_FOG && defined SKY_HORIZON_FOG
		// Apply horizon fog to sky to match terrain border fog
		// This creates a seamless transition between distant terrain and sky
		...
		float horizon_factor = linear_step(0.1, 1.0, exp(-75.0 * sqr(adjusted_sun_dir.y + 0.0496)));
		...
		vec3 sky_fog_color = mix(atmosphere, horizon_color, sqr(horizon_factor)) * (1.0 - biome_cave);

		// Blend sky toward fog color when looking near the horizon
		float sky_horizon_blend = dampen(linear_step(0.1, 0.0, abs(direction_world.y)));
		fragment_color = mix(fragment_color, sky_fog_color, sky_horizon_blend * SKY_HORIZON_FOG_INTENSITY);
#endif
        ...
        shadows = calculate_shadows(position_scene, flat_normal, light_levels.y, cloud_shadows, material.sss_amount, shadow_distance_fade, sss_depth, adjusted_light_dir);
