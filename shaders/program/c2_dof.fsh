...
void main() {
	...
	// For auto-focus, sample depth at screen center (where crosshair is)
	float focus;
	if (DOF_FOCUS < 0.0) {
		// Sample directly at center of render buffer - no blending, just the raw depth
		vec2 center_uv = vec2(0.5) * taau_render_scale;
		focus = textureLod(depthtex0, center_uv, 0).x;
	} else {
		focus = view_to_screen_space_depth(gbufferProjection, DOF_FOCUS);
	}
