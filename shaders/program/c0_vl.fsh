...
#if defined SHADOW_COLOR || defined AIR_FOG_COLORED_LIGHT_SHAFTS
	...
	switch (isEyeInWater) {
		...
		case 1:
			...
			// Simulates churned sediment, reduced clarity, and overcast conditions above
			{
				...
				float luma = dot(fog_scattering, luminance_weights);
				vec3 desat_target = luma * vec3(0.85, 0.92, 1.0); // Slightly cool grey
				float desat_strength = sqr(storm_intensity) * 0.5;
				fog_scattering = mix(fog_scattering, desat_target, desat_strength);
			}
#endif
