...
#ifdef LATITUDE_SUN_PATH
	vec3 lat_adjusted_sun_dir = get_latitude_adjusted_sun_dir(sun_dir, cameraPosition.z, yearProgress);
	vec3 lat_adjusted_light_dir = sunAngle < 0.5 ? lat_adjusted_sun_dir : -lat_adjusted_sun_dir;
#else
	#define lat_adjusted_light_dir light_dir
#endif
...
#ifndef NO_NORMAL
	float NoL = dot(normal, lat_adjusted_light_dir);
#else
	float NoL = 1.0;
#endif
	float NoV = clamp01(dot(normal, -world_dir));
	float LoV = dot(lat_adjusted_light_dir, -world_dir);
