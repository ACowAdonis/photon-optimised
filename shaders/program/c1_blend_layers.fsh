...
// Cold Sweat temperature (from modified Oculus)
...
#ifdef HEAT_HAZE
#include "/include/misc/heat_haze.glsl"
#endif
...
void main() {
	...
	// Heat haze distortion (ambient environmental heat only)
#ifdef HEAT_HAZE
	{
		float heat_intensity = 0.0;

		// Only cancel heat haze if player is underwater
		if (isEyeInWater != 1) {
			#if defined WORLD_NETHER
			// Nether: always has ambient heat (constant high temperature)
			heat_intensity = get_ambient_heat_intensity(uv, view_distance, 0.25, 10.0, direction_world, sun_dir, true, frameTimeCounter, noisetex);
			#elif defined WORLD_OVERWORLD
			// Heat haze based on Cold Sweat world temperature
			// Temperature already accounts for time, weather, indoor/outdoor
			heat_intensity = get_ambient_heat_intensity(uv, view_distance, sunAngle, worldAmbientTemp, direction_world, sun_dir, false, frameTimeCounter, noisetex);
			#endif
		}

		// Apply heat haze distortion
		if (heat_intensity > 0.001) {
			vec2 haze_offset = get_heat_haze_offset(uv, heat_intensity, frameTimeCounter, noisetex);
			refracted_uv += haze_offset;
		}
	}
#endif
	...
	if (front_depth_dh != back_depth_dh) {
		...
		if (front_is_dh_terrain || dh_behind_translucent) {
			...
			if (material_mask == MATERIAL_WATER) { // Water
				// OPT: Calculate frozen biome factor for wave reduction in frozen oceans
	...
	if (isEyeInWater == 1) {
		...
		// Storm desaturation for underwater: shift water color toward grey during storms
		// Simulates churned sediment, reduced clarity, and overcast conditions above
		{
			float storm_intensity = rainStrength;
			float luma = dot(analytic_fog[0], luminance_weights);
			vec3 desat_target = luma * vec3(0.85, 0.92, 1.0); // Slightly cool grey
			float desat_strength = sqr(storm_intensity) * 0.5;
			analytic_fog[0] = mix(analytic_fog[0], desat_target, desat_strength);
		}
		...
		#ifdef BLOOMY_FOG
		bloomy_fog = clamp01(dot(analytic_fog[1], vec3(luminance_weights_rec2020)));
		bloomy_fog = isEyeInWater == 1.0 ? sqrt(bloomy_fog) : bloomy_fog;
		#endif
	#else 
		#ifdef BLOOMY_FOG
