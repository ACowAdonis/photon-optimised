...
void main() {
	...
	// Lightning flash - natural eye adaptation effect
	// Only applies when player can see the sky (not in caves/buildings)
#ifdef LIGHTNING_FLASH
	if (LIGHTNING_FLASH_UNIFORM > 0.01) {
		// Visibility check: flash intensity based on what player can actually see
		// - eye_skylight: 0 = no sky visible, 1 = full sky exposure
		// - biome_cave: 0 = surface, 1 = underground
		float sky_visibility = eye_skylight * (1.0 - biome_cave * 0.9);

		// Instant flash - brief but impactful (pow 6.0 = fast but not instant)
		float instant = pow(LIGHTNING_FLASH_UNIFORM, 6.0);

		// Ambient component - subtle lingering glow
		float ambient = pow(LIGHTNING_FLASH_UNIFORM, 2.5) * 0.2;

		// Combined flash intensity, scaled by visibility
		float flash = (instant * 1.2 + ambient) * sky_visibility;

		// Skip the effect entirely if visibility is negligible
		if (flash > 0.001) {
			// Natural eye response:
			// 1. Exposure boost (multiplicative) - brightens the scene naturally
			float exposure_boost = 1.0 + flash * lightning_flash_intensity * 0.12;
			scene_color *= exposure_boost;

			// 2. Desaturation - bright light washes out color perception
			float desat_amount = instant * sky_visibility * 0.4;
			float luminance = dot(scene_color, vec3(0.2126, 0.7152, 0.0722));
			scene_color = mix(scene_color, vec3(luminance), desat_amount);

			// 3. Subtle cool tint shift
			scene_color *= mix(vec3(1.0), vec3(0.96, 0.98, 1.0), instant * sky_visibility * 0.6);
		}
	}
#endif
