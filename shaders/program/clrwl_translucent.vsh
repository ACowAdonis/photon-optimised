...
void main() {
	...
	#endif
	{
		// Make lightning bolt thinner - scale geometry inward toward center
		// This creates a tighter core that relies more on bloom for visual impact
		vec3 camera_to_vertex = normalize(position_scene);
		vec3 up = vec3(0.0, 1.0, 0.0);
		vec3 right = normalize(cross(camera_to_vertex, up));

		// Scale horizontal offset inward (0.4 = 40% of original width)
		float horizontal_offset = dot(position_scene, right);
		const float bolt_thinness = 0.4;
		position_scene -= right * horizontal_offset * (1.0 - bolt_thinness);
	}
#endif
