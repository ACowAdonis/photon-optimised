...
// Combined reinhard tonemapping + YCoCg conversion for TAA optimization
// Eliminates redundant function calls by fusing operations
vec3 reinhard_to_ycocg(vec3 rgb) {
	vec3 r = rgb / (rgb + 1.0);  // reinhard
	return vec3(
		0.25 * r.r + 0.5 * r.g + 0.25 * r.b,
		0.5 * r.r - 0.5 * r.b,
		-0.25 * r.r + 0.5 * r.g - 0.25 * r.b
	);
}
