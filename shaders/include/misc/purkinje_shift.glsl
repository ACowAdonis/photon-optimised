...
vec3 purkinje_shift(vec3 rgb, vec2 light_levels) {
	...
	// Reduce Purkinje during storms - storm desaturation already provides cool shift
	// Prevents excessive blue compounding from both effects
	purkinje_intensity *= 1.0 - 0.5 * sqr(rainStrength * STORM_INTENSITY_MULT);
#endif
