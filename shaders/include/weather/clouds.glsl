...
CloudsParameters get_clouds_parameters(Weather weather) {
	...
	// Storm cloud darkening: increased absorption and reduced scattering
	// Apply daily storm cap if available (limits how intense this rain event can become)
#ifdef STORM_DAILY_CAP_AVAILABLE
	float storm_intensity = min(rainStrength * STORM_INTENSITY_MULT, storm_daily_cap);
#else
	float storm_intensity = rainStrength * STORM_INTENSITY_MULT;
#endif
	float cloud_darkening = linear_step(0.25, 0.8, storm_intensity) * STORM_CLOUD_DARKENING;

	// Darken clouds by reducing scattering (clouds absorb more light)
	params.l0_scattering_coeff *= 1.0 - 0.4 * cloud_darkening;

	// Increase coverage during storms (more clouds form)
	params.l0_coverage += vec2(0.15, 0.25) * storm_intensity;
	params.l0_coverage = clamp01(params.l0_coverage);

	// Also increase L1 coverage for more dramatic storm atmosphere
	params.l1_coverage += vec2(0.1, 0.2) * storm_intensity;
	params.l1_coverage = clamp01(params.l1_coverage);
#endif
