...
OverworldFogParameters get_fog_parameters(Weather weather) {
	...
	// rain (humidity effect halved from original)
	params.rayleigh_scattering_coeff = mix(
		params.rayleigh_scattering_coeff * (1.0 + 0.5 * weather.humidity * weather.temperature),
		rayleigh_rain,
	...
	// Weather influence (humidity mie effect halved from 8.0 to 4.0)
	mie = mix(
		mie + 4.0 * AIR_FOG_MIE_DENSITY_NOON * sqr(clamp01(weather.humidity * rcp(0.8))),
