...
#if defined WAVING_PLANTS || defined WAVING_LEAVES
// OPT-002: Wind animation with distance-based LOD
// wind_lod: 1.0 = full quality, 0.0 = no animation
vec3 get_wind_displacement(vec3 world_pos, float wind_speed, float wind_strength, bool is_tall_plant_top_vertex, float wind_lod) {
	// Early exit if LOD is too low (distant vegetation)
	if (wind_lod < 0.01) return vec3(0.0);
	...
	// Wobble calculation with LOD - skip higher frequency terms at distance
	world_pos = 32.0 * world_pos + 3.0 * t + vec3(0.0, golden_angle, 2.0 * golden_angle);
	vec3 wobble = sin(world_pos);
	// Only add higher frequency wobble at close range (LOD > 0.5)
	if (wind_lod > 0.5) {
		wobble += 0.5 * sin(2.0 * world_pos) + 0.25 * sin(4.0 * world_pos);
	}

	if (is_tall_plant_top_vertex) { gust *= 2.0; wobble *= 0.5; }

	return wind_strength * (gust + 0.1 * wobble) * wind_lod;
}
#endif
...
vec3 animate_vertex(vec3 world_pos, bool is_top_vertex, float skylight, uint material_mask) {
	...
	// OPT-002: Calculate wind LOD based on distance from camera
	float dist = length(world_pos - cameraPosition);
	float wind_lod = smoothstep(64.0, 16.0, dist);
	...
	case MATERIAL_OPEN_EYEBLOSSOM:
		return world_pos + (get_wind_displacement(world_pos, wind_speed, wind_strength, false, wind_lod) + player_displacement) * float(is_top_vertex);

	case MATERIAL_TALL_PLANTS_LOWER:
		return world_pos + (get_wind_displacement(world_pos, wind_speed, wind_strength, false, wind_lod) + player_displacement) * float(is_top_vertex);

	case MATERIAL_TALL_PLANTS_UPPER:
		return world_pos + (get_wind_displacement(world_pos, wind_speed, wind_strength, is_top_vertex, wind_lod) + player_displacement);
#endif

#ifdef WAVING_LEAVES
	case MATERIAL_LEAVES:
		return world_pos + get_wind_displacement(world_pos, wind_speed, wind_strength * 0.5, false, wind_lod);
#endif
