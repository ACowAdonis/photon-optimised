...
mat2x3 raymarch_air_fog(vec3 world_start_pos, vec3 world_end_pos, bool sky, float skylight, float dither) {
	...
	for (int i = 0; i < step_count; ++i, world_pos += world_step, shadow_pos += shadow_step) {
		...
		// Blend shadow toward 1.0 (no shadow effect) as rain intensity increases
	#ifdef AIR_FOG_COLORED_LIGHT_SHAFTS
		vec3 weather_shadow = mix(shadow, vec3(1.0), rainStrength);
	#else
		float weather_shadow = mix(shadow, 1.0, rainStrength);
	#endif

		light_sun[0] += visible_scattering * density.x * weather_shadow;
		light_sun[1] += visible_scattering * density.y * weather_shadow;
		light_sky[0] += visible_scattering * density.x * weather_shadow;
		light_sky[1] += visible_scattering * density.y * weather_shadow;
