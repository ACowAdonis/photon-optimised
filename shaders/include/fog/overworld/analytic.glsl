...
mat2x3 air_fog_analytic(vec3 ray_origin_world, vec3 ray_end_world, bool sky, float skylight, float shadow) {
	...
	scattering += 2.0 * (rayleigh_scattering + mie_scattering) * isotropic_phase * ambient_color * weather_shadow;

	for (int i = 0; i < 4; ++i) {
		float mie_phase = 0.7 * henyey_greenstein_phase(LoV, 0.5 * anisotropy) + 0.3 * henyey_greenstein_phase(LoV, -0.2 * anisotropy);

		scattering += scatter_amount * (rayleigh_scattering * isotropic_phase + mie_scattering * mie_phase) * light_color * (1.0 - 0.9 * rainStrength) * weather_shadow;
