...
mat2x3 raymarch_water_fog(
	...
	for (int i = 0; i < step_count; ++i, world_pos += world_step, shadow_pos += shadow_step, caustics_pos += caustics_step) {
		...
		// Shadow affects scattering (line 103), not transmittance - light absorption
		// through water is independent of whether the light source is shadowed
		vec3 light_transmittance = exp(-extinction_coeff * distance_traveled);
