...
#ifdef SHADOW_VPS
// OPT-006: Simplified SSS for non-water translucents (foliage)
// Skips expensive phase function and sheen calculations
#if defined PROGRAM_GBUFFERS_BLOCK_TRANSLUCENT || defined PROGRAM_GBUFFERS_ENTITIES_TRANSLUCENT
vec3 sss_approx(
	vec3 albedo,
	float sss_amount,
	float sheen_amount,
	...
	// Simplified coefficient - skip inversesqrt normalization
	float avg_albedo = dot(albedo, vec3(0.333));
	float coeff = (1.0 - 0.75 * avg_albedo) * sss_density / sss_amount;

	// Use constant isotropic phase instead of expensive henyey_greenstein
	vec3 sss = sss_scale * isotropic_phase * exp2(-coeff * sss_depth) * dampen(sss_amount) * pi;
...
#if defined WORLD_OVERWORLD
	// In overcast/stormy weather, light is diffused through clouds - no distinct shadows
	// Only apply to surfaces with skylight exposure - caves remain unaffected
	// Use smoothstep threshold to avoid discontinuities under partial cover outdoors
	float has_sky_exposure = smoothstep(0.0, 0.2, light_levels.y);
	shadows = mix(shadows, vec3(1.0), rainStrength * has_sky_exposure);
#endif

	// OPT-004: Fade SSS at distance to reduce computation for distant foliage
	float sss_distance_fade = smoothstep(48.0, 32.0, length(scene_pos));
	material.sss_amount *= sss_distance_fade;
...
#ifdef CLOUD_SHADOWS
	// Reduce cloud shadow effect during storms (diffused light doesn't cast distinct shadows)
	lighting *= mix(cloud_shadows, 1.0, rainStrength);
#endif
#endif
...
#if defined WORLD_OVERWORLD
	// biome_cave: 0 = player on surface, 1 = player underground
	// Use this to distinguish between surface shade (jungle canopy) and actual caves

	// Cave lighting - applies when player is underground
	// biome_cave scales this so it only affects underground areas
	float cave_shade = (1.0 - light_levels.y * light_levels.y);
	lighting += 0.15 * CAVE_LIGHTING_I * directional_lighting * ao * cave_shade * biome_cave * (1.0 - 0.7 * darknessFactor);

	// Surface shade ambient - applies when player is on surface
	// Provides ambient fill for ANY shaded area (dense jungle, overhangs) when above ground
	// (1.0 - biome_cave) ensures this only applies on surface, not in caves
	float surface_factor = 1.0 - biome_cave;
	float shade_amount = 1.0 - light_levels.y;  // More ambient where less skylight
	lighting += 0.3 * SURFACE_SHADE_BRIGHTNESS * directional_lighting * ao * shade_amount * surface_factor * ambient_color;
