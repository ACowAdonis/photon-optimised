...
// Shadow PCF settings for solid terrain
...
// OPT-003: Reduced shadow PCF for translucent surfaces (water, remaining foliage)
// Translucent surfaces are less visually sensitive to shadow quality
#if defined PROGRAM_GBUFFERS_WATER || defined PROGRAM_GBUFFERS_BLOCK_TRANSLUCENT || defined PROGRAM_GBUFFERS_ENTITIES_TRANSLUCENT
	#undef SHADOW_PCF_STEPS_MIN
	#undef SHADOW_PCF_STEPS_MAX
	#define SHADOW_PCF_STEPS_MIN 4
	#define SHADOW_PCF_STEPS_MAX 6
#endif
...
#ifdef SHADOW
vec2 blocker_search(vec3 scene_pos, float dither, bool has_sss, vec3 light_direction) {
	// OPT-011: Reduced blocker search steps for non-SSS materials (3 -> 2)
	uint step_count = has_sss ? SSS_STEPS : 2;
	...
	float radius = SHADOW_BLOCKER_SEARCH_RADIUS * shadowProjection[0].x * (0.5 + 0.5 * linear_step(0.2, 0.4, light_direction.y));
...
vec3 calculate_shadows(
	...
	out float sss_depth,
	vec3 light_direction  // Use adjusted_light_dir when LATITUDE_SUN_PATH is enabled
) {
	sss_depth = 0.0;
	distance_fade = 0.0;

	float NoL = dot(flat_normal, light_direction);
	// OPT-001: Early exit for backlit surfaces with low SSS
	// Original threshold was 1e-3, but surfaces facing away from light (NoL < -0.5)
	// with minimal SSS don't need shadow calculations at all
	if (NoL < -0.5 && sss_amount < 0.1) return vec3(0.0);
	...
	distance_fade = pow32(
		max(
			...
			mix(
				1.0, 0.55,
				linear_step(0.33, 0.8, light_direction.y)
...
#ifdef SHADOW_VPS
	vec2 blocker_search_result = blocker_search(scene_pos, dither, sss_amount > eps, light_direction);
