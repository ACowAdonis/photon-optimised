...
vec3 get_specular_reflections(
	...
	// OPT-012: Early exit for very rough non-water surfaces where reflections are barely visible
	if (material.roughness > 0.8 && !is_water) return vec3(0.0);
	...
	// Early-out for non-reflective surfaces to save 10-15% performance
	if (!is_water && material.ssr_multiplier > 0.01) { // Rough reflection
