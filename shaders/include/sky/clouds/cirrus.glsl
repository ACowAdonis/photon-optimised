...
#if defined LATITUDE_SUN_PATH && defined PROGRAM_DEFERRED1
	float adjusted_sun_y = get_latitude_adjusted_sun_dir(sun_dir, cameraPosition.z, yearProgress).y;
#else
	float adjusted_sun_y = sun_dir.y;
#endif
	float scatter_amount = clouds_cirrus_scattering_coeff;
	float extinct_amount = clouds_cirrus_extinction_coeff * (1.0 + 0.5 * max0(smoothstep(0.0, 0.15, abs(adjusted_sun_y)) - smoothstep(0.5, 0.7, clouds_params.cirrus_amount)));
...
#if defined LATITUDE_SUN_PATH && defined PROGRAM_DEFERRED1
	vec3 cloud_sun_dir = get_latitude_adjusted_sun_dir(sun_dir, cameraPosition.z, yearProgress);
	vec3 cloud_moon_dir = -cloud_sun_dir;
#else
	vec3 cloud_sun_dir = sun_dir;
	vec3 cloud_moon_dir = moon_dir;
#endif
	bool moonlit = cloud_sun_dir.y < -0.049;
	vec3 light_dir = moonlit ? cloud_moon_dir : cloud_sun_dir;
