...
vec4 draw_crepuscular_rays(
	...
	// Fade rays when light source is near or below horizon
	// light_dir.y < 0 means below horizon, fade out smoothly
	float horizon_fade = smoothstep(-0.05, 0.1, light_dir.y);
	...
	scattering *= (6.0 * CREPUSCULAR_RAYS_INTENSITY) * phase * horizon_fade;
